# Bank Statement PDF Parser (HDFC / SBI / IOB / Canara / Indian Bank)

A high-performance parser to convert **HDFC**, **SBI**, **Indian Overseas Bank**, **Canara Bank**, and **Indian Bank** PDF statements into clean, structured CSV and Excel (.xlsx) formats. Handles multi-line narrations, page-boundary overflows, reverse-chronological ordering, and mixed withdrawal/deposit alignments with **100% mathematical consistency**.
...
**Auto-detection:** The parser reads page 1 and detects the bank automatically. Pass `--bank hdfc|sbi|iob|canara|indianbank` to override.

| Option | Description |
|--------|-------------|
| `--csv PATH` | Specify custom output path for CSV |
| `--xlsx PATH` | Specify custom output path for Excel |
| `--bank hdfc\|sbi\|iob\|canara\|indianbank` | Manually specify bank (auto-detected by default) |
